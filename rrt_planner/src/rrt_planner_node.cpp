...
int main(int argc, char **argv)
{
	...
	nh.getParam("goal_position", goal_position);					// x,y goal_position in UTM frame
	...
	else
	{
		...
		path_utm.header.frame_id = "odom";
		path_utm.poses.resize(goal_position.size()/2);
		for (int i=0; i<goal_position.size()/2; i++)
		{
			path_utm.poses[i].header = path_utm.header;
			path_utm.poses[i].pose.position.x = goal_position[2*i+0];
			path_utm.poses[i].pose.position.y = goal_position[2*i+1];
	...
	while (ros::ok())
	{
		...
		ros::Duration map_wait = ros::Time::now() - map_wait_start;
		ROS_INFO_STREAM(map_wait.toSec());
		...
		// for (int temp_i = -16; temp_i <= 16; temp_i++)
		// 	for(int temp_j = -16; temp_j <= 16; temp_j++)
		// 		cfg_space.data[(root->y+temp_i)*cfg_space.info.width + (root->x+temp_j)] = 0;
		...
		ros::Duration rrt_duration = ros::Time::now() - rrt_start;
		ROS_INFO_STREAM(rrt_duration.toSec());
		...
		if (min_distance > 0)
		{
			...
			// (also updates goal->P)
			if (angleCheck(*(goal->parent), *(goal), max_angle) && localPlanner(*(goal->parent), *(goal), cfg_space))
			{
				all_checks_passed = true;
			}
		}
...
void randSample(Node& sample, const nav_msgs::OccupancyGrid& cfg_space, const Node& centre)
{
	...
	while(true)
	{
		...
		if (0 <= sample.x && sample.x < map_width &&
			0 <= sample.y && sample.y < map_height)
